...
// Seeded PRNG — mulberry32. Seed fixed at 42 for determinism.
...
export function computeClusters(
  ...
  for (const [, group] of byDomain) {
    ...
    let assignments: number[] = new Array<number>(n).fill(-1);
    for (let iter = 0; iter < 20; iter++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (cCount !== undefined) counts[c] = cCount + 1;
        const cRow = sums[c];
        const tEmbed = sorted[i]!.centroid;
        if (cRow !== undefined) {
          for (let j = 0; j < dim; j++) {
            cRow[j] = (cRow[j] ?? 0) + (tEmbed[j] ?? 0);
          }
        }
      }
      for (let c = 0; c < k; c++) {
        ...
        const cRow = sums[c];
        if (cnt > 0 && cRow !== undefined) {
          for (let j = 0; j < dim; j++) {
            cRow[j] = (cRow[j] ?? 0) / cnt;
          }
        } else if (cRow !== undefined) {
