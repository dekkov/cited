...
**Domain:** OSS evidence-backed health habit tracker (web app + admin + RAG-driven AI onboarding)
**Researched:** 2026-05-07
**Overall confidence:** HIGH on web/db/auth core, MEDIUM on background-job and docs choices

> **Reading note for the roadmap author:** A "starting hypothesis" stack was captured in `PROJECT.md` and a prior market-validation artifact. This document validates that hypothesis against current 2026 sources. Where the artifact still holds, that's stated explicitly. Where it has drifted (versions, ecosystem shifts, license changes), the recommendation is updated and the rationale called out. The most consequential drift is **Next.js 16 is the current stable** (not 15), and the **Auth.js team merged into Better Auth in Sept 2025** — neither changes the recommended choice for *this* project, but both deserve a roadmap callout.

---

## Self-Host Friction Budget

This is an OSS project with `docker compose up` as a stated goal. Every dependency that can't be self-hosted, or requires a separate vendor account to run locally, is a tax on contributors and self-hosters. Stack decisions below explicitly weigh self-host friction.
...
**Net assessment:** The Supabase-as-platform decision is the largest single self-host cost. It's still the right call (auth + db + storage + RLS in one box, with a documented compose stack), but the architecture must keep direct Supabase coupling at the edges (auth callback, storage uploads) so a "pure Postgres" fallback is plausible later.
...
# Monorepo bootstrap
pnpm init
pnpm add -D turbo typescript @biomejs/biome husky lint-staged

# apps/web (and apps/admin)
pnpm --filter web add next@16 react@19 react-dom@19
...
pnpm --filter web add ai @ai-sdk/openai @ai-sdk/anthropic
pnpm --filter web add @next/third-parties
pnpm --filter web add lucide-react resend react-email
pnpm --filter web add -D drizzle-kit tailwindcss@4 @tailwindcss/postcss
...
# Testing
pnpm --filter web add -D vitest @testing-library/react @testing-library/jest-dom playwright
```
...
**If MVP / hand-curated clips only (Phase 1):**
...
**If extraction pipeline lands (Phase 2 / v0.5):**
...
For the roadmap author: where does this STACK.md disagree with the prior artifact, and how serious?
...
Confidence on most-cited claims is HIGH — multiple 2026-dated sources converge on the same versions and recommendations.

---
*Stack research for: OSS evidence-backed health habit tracker (Hdiary)*
*Researched: 2026-05-07*
